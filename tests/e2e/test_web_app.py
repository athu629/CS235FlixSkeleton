...
def test_index(client):
    ...
    assert b'Movie Browser CS235' in response.data


def test_movies_without_title(client):
    # Check that we can retrieve the movies page.
    response = client.get('/retrieve_movie')
    assert response.status_code == 200

    # check that without providing a movie_id query parameter the page includes the first movie
    assert b'Guardians of the Galaxy' in response.data


def test_movies_with_title(client):
    # check that we can retrieve the movies page.
    response = client.get('/retrieve_movie?target_id=Mindhorn2016')
    assert response.status_code == 200

    # check that the the requested movie is on the page.
    assert b'Mindhorn' in response.data
    assert b'2016' in response.data
    assert b'A has-been actor best known for playing the' in response.data
...
def test_movies_with_genre(client):
    ...
    assert b'Guardians of the Galaxy' in response.data
    assert b'A group of intergalactic criminals are forced' in response.data
...
def test_review(client, auth):
    ...
    response = client.get('/review?movie=Split2014')

    response = client.post(
        '/review',
        data={'review': 'Creepy how many personalities?', 'movie_id': 'Split2014'}
    )
...
def test_review_with_invalid_input(client, auth, review, messages):
    ...
    response = client.post(
        '/review',
        data={'review': review, 'movie_id': 'Split2014'}
    )
